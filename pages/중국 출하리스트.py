...
for _, row in filtered_df.iterrows():
    eta = row["회사도착 예상일(=ETA+1)"]
    if pd.notna(eta):
        color = "#2ECC71" if "도착" in row["상태표시"] else "#E74C3C"
        events.append({
            "title": f"{row['PRODUCT']} - {row['상태표시']}",
...
if not matched.empty:
    st.markdown("---")
    st.markdown(f"## 📦 {selected_date} 도착 예정 상품 목록")
    cols = st.columns(3)
    for i, (_, row) in enumerate(matched.iterrows()):
        border_color = get_border_color(row["D-Day"])
        with cols[i % 3]:
            st.markdown(f"""
            <div style="border:3px solid {border_color}; border-radius:14px; padding:20px; margin:10px; background-color:#fffbea;">
                ...
                <div style="line-height:1.8; font-size:16px;">
                    ...
                    <div>🗓 <strong>ETA+1:</strong> {row['회사도착 예상일(=ETA+1)'].date()}</div>
                    <div>🚚 <strong>도착여부:</strong> {row['도착여부']}</div>
                    <div>📆 <strong>D-Day:</strong> {row['D-Day']}</div>
                </div>
            </div>
...
for _, row in filtered_df.sort_values("회사도착 예상일(=ETA+1)").iterrows():
    eta = row["회사도착 예상일(=ETA+1)"]
    d_day = row["D-Day"]
    ...
    <div style="border:3px solid {border_color}; border-radius:14px; padding:26px; margin-bottom:22px; background-color:#fefefe;">
        ...
        <div style="line-height:2.1; font-size:20px; margin-top:16px;">
            ...
            <div>📦 <strong>상태:</strong> {row['상태표시']}</div>
            <div>🗓 <strong>ETA+1:</strong> {eta.date()}</div>
            <div>🚚 <strong>도착여부:</strong> {row['도착여부']}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
